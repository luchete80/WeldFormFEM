...
contains 
  ...
  !!!!! NODE DISTRIBUTION ARE LIKE IN FLANAGAN (1981), GOUDREAU, AND BENSON (1992)
  subroutine AddBoxLengthTri(tag, V, Lx, Ly, Lz, r, Density,  h)			
    integer, intent(in):: tag
  ...
  !end do
    ...
    call AllocateElements(2 * nel(1) * nel(2),gp) !!!!REDUCED INTEGRATION
    ...
    do while ( ey < nel(2))
        ...
        do while (ex < nel(1)) 
            !!elem%elnod(i,:)=[(nel(1)+1)*ey + ex+1,(nel(1)+1)*ey + ex+2,(nel(1)+1)*(ey+1)+ex+2,(nel(1)+1)*(ey+1)+ex+1]         
            elem%elnod(i  ,:)=[(nel(1)+1)*ey + ex+1,(nel(1)+1)*ey + ex+2,(nel(1)+1)*(ey+1)+ex+1] !!ndoes 1,2,3
            elem%elnod(i+1,:)=[(nel(1)+1)*ey + ex+2,(nel(1)+1)*(ey+1)+ex+2,(nel(1)+1)*(ey+1)+ex+1] !!NODES 2,4,3
            print *, "Element ", i , "Elnod", elem%elnod(i,:) 
            i=i+2
          ex = ex + 1
        end do
    ...
    call SearchNodelem
  end subroutine AddBoxLengthTri 
